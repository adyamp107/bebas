...
extension CameraViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
    nonisolated public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        do {
            ...
            let sortedObservations = self.sortHandsByXPosition(observations)
    ...
    private func sortHandsByXPosition(_ observations: [VNHumanHandPoseObservation]) -> [VNHumanHandPoseObservation] {
                guard observations.count == 2,
                      let wrist1 = try? observations[0].recognizedPoint(.wrist),
                      let wrist2 = try? observations[1].recognizedPoint(.wrist) else {
                    return observations
                }
                return wrist1.location.x < wrist2.location.x ? observations : [observations[1], observations[0]]
            }
}
